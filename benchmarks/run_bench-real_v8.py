...
#  
#  Since MPAS-A version 8.0, init_atmosphere can be run with multi threads.
#  This version was written for implementing this capability.
#-------------------------------------------------------------------------------
...
if args.run:
    ...
    mainexec = f"mpiexec -n {str(args.threads)} ./atmosphere_model"

else:
    benchs = glob.glob(b_dir+"/init*")
    mainexec = f"mpiexec -n {str(args.threads)} ./init_atmosphere_model"
