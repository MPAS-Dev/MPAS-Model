!  SVN:$Id: ice_algae_modified.F90 1183 2017-03-16 19:50:34Z njeffery $
!=======================================================================
!
...
!
      module ice_algae_modified

      use ice_kinds_mod_modified
      use ice_zbgc_shared_modified
...
!=======================================================================
      ...
      use ice_aerosol_modified, only: update_snow_bgc
      use ice_constants_colpkg_modified, only: c0, c1, puny
      use ice_zbgc_modified, only: merge_bgc_fluxes
...
!=======================================================================
      ...
      use ice_zbgc_modified, only: merge_bgc_fluxes_skl
      use ice_colpkg_tracers_modified, only: nt_bgc_N
...
! 
      ...
      use ice_constants_colpkg_modified, only: p5, p05, p1, c1, c0, puny, c10
      use ice_colpkg_tracers_modified, only: nt_bgc_N,  ntrcr, bio_index 
      use ice_colpkg_shared_modified, only: dEdd_algae, bgc_flux_type, sk_l, R_chl2N
...
! 
      ...
      use ice_colpkg_tracers_modified, only: nt_fbri, nt_zbgc_frac, &
                                    ntrcr, nlt_bgc_Nit, tr_bgc_Fe, tr_zaero, &
                                    nlt_bgc_Fed, nlt_zaero, bio_index, tr_bgc_N, &
                                    nlt_bgc_N
      use ice_constants_colpkg_modified, only: c0, c1, c2, p5, puny, pi
      use ice_colpkg_shared_modified, only: hi_ssl, dEdd_algae, solve_zbgc, &
...
!          Nicole Jeffery, LANL
      ...
      use ice_constants_colpkg_modified, only: p1, p5, c0, c1, secday, puny
      use ice_colpkg_shared_modified, only: max_algae, max_DON, max_DOC, R_C2N, R_chl2N, &
                                   T_max, fsal      , fr_resp          , &
      ...
      use ice_zbgc_shared_modified, only:   chlabs, alpha2max_low, beta2max, mu_max, &
                                   grow_Tdep, fr_graze, mort_pre, mort_Tdep, &
                                   k_exude, K_Nit, K_Am, K_Sil, K_Fe, &   
                                   f_don, kn_bac, f_don_Am, & 
                                   f_doc, f_exude, k_bac

      use ice_colpkg_tracers_modified, only: tr_brine, nt_fbri, &
...
! authors     Nicole Jeffery, LANL
      ...
      use ice_constants_colpkg_modified, only: c1, p5, c0
...
!
      subroutine compute_FCT_matrix &
       ...
       use ice_constants_colpkg_modified, only: c1, c0, p5, c2, puny
       use ice_colpkg_shared_modified, only: grid_o
...
!
      subroutine compute_FCT_corr &
       ...
       use ice_constants_colpkg_modified, only: c1, c0, c6, puny
...
! authors     Nicole Jeffery, LANL
      ...
      use ice_constants_colpkg_modified, only: p5, c1, c4, c0
...
! author: Nicole Jeffery, LANL
      ...
      use ice_colpkg_shared_modified, only: hs_ssl 
      use ice_constants_colpkg_modified, only: p5, c1, c0
