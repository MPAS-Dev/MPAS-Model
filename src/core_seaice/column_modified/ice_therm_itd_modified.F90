!  SVN:$Id: ice_therm_itd_modified.F90 1196 2017-04-18 13:32:23Z eclare $
!=======================================================================
!
...
!       First ice_therm_vertical_modified computes vertical growth rates and coupler
!       fluxes.  Then ice_therm_itd_modified does thermodynamic calculations not
!       needed for coupling.
!       
...
!
      module ice_therm_itd_modified

      use ice_kinds_mod_modified
      use ice_constants_colpkg_modified, only: c0, c1, c2, c3, c4, c6, c10, &
...
!          Elizabeth C. Hunke, LANL
      ...
      use ice_itd_modified, only: aggregate_area, shift_ice, & 
                         column_sum, column_conservation_check
      use ice_colpkg_tracers_modified, only: nt_qice, nt_qsno, nt_fbri, nt_sice, &
...
!
      ...
      use ice_colpkg_tracers_modified, only: nt_qice, nt_qsno, nt_aero, tr_aero, &
                             tr_pond_topo, nt_apnd, nt_hpnd, bio_index
      use ice_colpkg_shared_modified, only: z_tracers , hs_ssl, solve_zsal
      use ice_zbgc_modified, only: lateral_melt_bgc               
...
!
      ...
      use ice_itd_modified, only: column_sum, &
                         column_conservation_check 
      use ice_colpkg_tracers_modified, only: nt_Tsfc, nt_iage, nt_FY, nt_sice, nt_qice, &
                             nt_alvl, nt_vlvl, nt_aero, nt_apnd, &
                             tr_pond_cesm, tr_pond_lvl, tr_pond_topo, &
                             tr_iage, tr_FY, tr_lvl, tr_aero, tr_brine
      use ice_colpkg_shared_modified, only: solve_zsal, skl_bgc, initbio_frac, salt_loss, rhosi
      use ice_mushy_physics_modified, only: liquidus_temperature_mush, enthalpy_mush
      use ice_therm_shared_modified, only: hfrazilmin
      use ice_zbgc_modified, only: add_new_ice_bgc
      use ice_zbgc_shared_modified, only: bgc_tracer_type
...
!=======================================================================
      ...
      end module ice_therm_itd_modified
