...
! authors: Elizabeth C. Hunke, LANL
      ...
      module ice_colpkg_modified

      use ice_kinds_mod_modified
      use ice_colpkg_shared_modified ! namelist and other parameters
      use ice_warnings_modified, only: add_warning
...
!          C. M. Bitz, UW
      ...
      use ice_colpkg_shared_modified, only: kcatbound, kitd
      use ice_therm_shared_modified, only: hi_min
      use ice_constants_colpkg_modified, only: p01, p1, c0, c1, c2, c3, c15, c25, c100
...
!          C. M. Bitz, UW
      ...
      use ice_colpkg_shared_modified, only: kcatbound, kitd
      use ice_constants_colpkg_modified, only: p01, p1, c2, c3, c15, c25, c100
...
!          William H. Lipscomb, LANL
      ...
      use ice_colpkg_shared_modified, only: saltmax, ktherm, heat_capacity, &
          min_salin
      use ice_constants_colpkg_modified, only: p5, c0, c1, c2, pi
      use ice_therm_shared_modified, only: l_brine
...
! author:  Bruce P. Briegleb, NCAR 
      ...
      use ice_constants_colpkg_modified, only: iyear_AD, eccen, obliqr, lambm0, &
         mvelpp, obliq, mvelp, decln, eccf, log_print

#ifdef CCSMCOUPLED
      use shr_orb_mod, only: shr_orb_params
#else
      use ice_orbital_modified, only: shr_orb_params
#endif
...
!=======================================================================
      ...
      use ice_colpkg_shared_modified, only: calc_Tsfc
      use ice_constants_colpkg_modified, only: Tsmelt, Tffresh, p5, cp_ice, cp_ocn, &
          Lfresh, rhoi, rhos, c0, c1
      use ice_mushy_physics_modified, only: enthalpy_mush
...
!=======================================================================
      ...
      use ice_constants_colpkg_modified, only: c0, c1, c2, p1, p15, p5
      use ice_zbgc_shared_modified, only: R_S2N, zbgc_frac_init, zbgc_init_frac, remap_zbgc

      ! column package includes
      use ice_colpkg_tracers_modified, only: nt_fbri, nt_bgc_S, nt_sice, nt_zbgc_frac, &
         bio_index_o,  bio_index  
      use ice_colpkg_shared_modified, only: solve_zsal, ktherm, hs_ssl,  &
...
!=======================================================================
      ...
      use ice_constants_colpkg_modified, only: c1, p5, c0, c2

      use ice_colpkg_shared_modified, only: &
         algaltype, doctype, dictype, dontype, fedtype, feptype, zaerotype, &
         R_C2N, R_chl2N, F_abs_chl, R_C2N_DON

      use ice_zbgc_shared_modified, only: zbgc_init_frac, &
...
!=======================================================================
      ...
      use ice_constants_colpkg_modified, only: c0, c1
...
!=======================================================================
      ...
      function colpkg_liquidus_temperature(Sin) result(Tmlt)
        ...
        use ice_colpkg_shared_modified, only: ktherm
        use ice_constants_colpkg_modified, only: depressT
        use ice_mushy_physics_modified, only: liquidus_temperature_mush
...
!=======================================================================
      ...
      function colpkg_sea_freezing_temperature(sss) result(Tf)
        ...
        use ice_colpkg_shared_modified, only: tfrz_option
        use ice_constants_colpkg_modified, only: depressT
...
!=======================================================================
      ...
      function colpkg_ice_temperature(qin, Sin) result(Tin)
        ...
        use ice_colpkg_shared_modified, only: ktherm
        use ice_constants_colpkg_modified, only: depressT
        use ice_mushy_physics_modified, only: temperature_mush
        use ice_therm_shared_modified, only: calculate_Tin_from_qin
...
!=======================================================================
      ...
      function colpkg_snow_temperature(qin) result(Tsn)
        ...
        use ice_colpkg_shared_modified, only: ktherm
        use ice_mushy_physics_modified, only: temperature_snow
        use ice_constants_colpkg_modified, only: Lfresh, rhos, cp_ice
...
!=======================================================================
      ...
      function colpkg_enthalpy_snow(zTsn) result(qsn)
        ...
        use ice_mushy_physics_modified, only: enthalpy_snow
...
!          Elizabeth C. Hunke, LANL
      ...
      use ice_aerosol_modified, only: update_aerosol
      use ice_atmo_modified, only: neutral_drag_coeffs
      use ice_age_modified, only: increment_age
      use ice_constants_colpkg_modified, only: rhofresh, rhoi, rhos, c0, c1, puny, &
          snwlvlfac
      use ice_firstyear_modified, only: update_FYarea
      use ice_flux_colpkg_modified, only: set_sfcflux, merge_fluxes
      use ice_meltpond_cesm_modified, only: compute_ponds_cesm
      use ice_meltpond_lvl_modified, only: compute_ponds_lvl
      use ice_meltpond_topo_modified, only: compute_ponds_topo
      use ice_snow_modified, only: drain_snow
      use ice_therm_shared_modified, only: hi_min
      use ice_therm_vertical_modified, only: frzmlt_bottom_lateral, thermo_vertical
      use ice_colpkg_tracers_modified, only: tr_iage, tr_FY, tr_aero, tr_pond, &
...
!          Elizabeth C. Hunke, LANL
      ...
      use ice_constants_colpkg_modified, only: puny
      use ice_itd_modified, only: aggregate_area, reduce_area, cleanup_itd
      use ice_therm_itd_modified, only: linear_itd, add_new_ice, lateral_melt
      use ice_colpkg_tracers_modified, only: ntrcr, nbtrcr, tr_aero, tr_pond_topo
...
! authors: Elizabeth Hunke, LANL
      ...
      use ice_constants_colpkg_modified, only: c0, c1, puny
...
!          Elizabeth C. Hunke, LANL
      ...
      use ice_constants_colpkg_modified, only: c0, puny
      use ice_shortwave_modified, only: run_dEdd, shortwave_ccsm3, compute_shortwave_trcr
      use ice_colpkg_tracers_modified, only: tr_pond_cesm, tr_pond_lvl, tr_pond_topo, &
                                    tr_bgc_N, tr_aero, tr_rsnw, tr_zaero

      use ice_colpkg_shared_modified, only:  z_tracers, skl_bgc
...
!          Elizabeth C. Hunke, LANL
      ...
      use ice_mechred_modified, only: ridge_ice
      use ice_itd_modified, only: cleanup_itd
      use ice_colpkg_tracers_modified, only: tr_pond_topo, tr_aero, tr_brine, ntrcr, nbtrcr
...
!          W. H. Lipscomb, LANL
      ...
      use ice_constants_colpkg_modified, only: c0, c1
      use ice_colpkg_tracers_modified, only: colpkg_compute_tracers
...
!          Elizabeth C. Hunke, LANL
      ...
      use ice_constants_colpkg_modified, only: p333, c0, c1, c2, Cf, Cp, Pstar, Cstar, &
          rhoi, puny
      use ice_mechred_modified, only: asum_ridging, ridge_itd
...
!=======================================================================
      ...
      use ice_atmo_modified, only: atmo_boundary_const, atmo_boundary_layer
      use ice_constants_colpkg_modified, only: c0
...
!       ice_therm_vertical_modified.
      ...
      use ice_constants_colpkg_modified, only: c0, c1, c1000, &
...
!          Nicole Jeffery, LANL
      ...
      use ice_colpkg_tracers_modified, only: tr_snow, tr_rsnw
      use ice_constants_colpkg_modified, only: c0, puny, rhos
      use ice_snow_modified, only: snow_effective_density, update_snow_radius, &
                          snow_redist
...
! set tracer active flags
      ...
      subroutine colpkg_init_tracer_flags(&
        ...
        use ice_colpkg_tracers_modified, only: &
...
!=======================================================================
      ...
      use ice_algae_modified, only: zbio, sklbio
      use ice_brine_modified, only: preflushing_changes, compute_microS_mushy, &
                           update_hbrine, compute_microS 
      use ice_colpkg_shared_modified, only: solve_zsal, z_tracers, phi_snow
      use ice_colpkg_tracers_modified, only: nt_fbri, tr_brine, &
          nt_bgc_S, nt_qice, nt_sice, nt_zbgc_frac, bio_index 
      use ice_constants_colpkg_modified, only: c0, c1, puny
      use ice_zsalinity_modified, only: zsalinity
      use ice_zbgc_shared_modified, only:  zbgc_frac_init
...
!  Initialize brine height tracer
      ...
      use ice_constants_colpkg_modified, only: c1, c1p5, c2, p5, c0, rhoi, rhos
      ...
      ! see ice_shortwave_modified.F90
...
!  Initialize ocean concentration
      ...
      use ice_constants_colpkg_modified, only: c1,  c2, p5, c0, p1
      use ice_colpkg_shared_modified, only: R_C2N, R_C2N_DON
...
!  Initialize zSalinity
      ...
      use ice_constants_colpkg_modified, only: c1,  c2, p5, c0, p1
      use ice_colpkg_shared_modified, only: dts_b, salt_loss
...
! basic initialization for ocean_bio_all
      ...
      use ice_constants_colpkg_modified, only: c0
      use ice_colpkg_shared_modified, only:  R_C2N, R_chl2N
      use ice_zbgc_shared_modified, only: R_S2N
...
!=======================================================================
      ...
      subroutine colpkg_clear_warnings()
        ...
        use ice_warnings_modified, only: reset_warnings
...
!=======================================================================
      ...
      subroutine colpkg_get_warnings(warningsOut)
        ...
        use ice_warnings_modified, only: &
             get_number_warnings, &
             get_warning
...
!=======================================================================
      ...
      end module ice_colpkg_modified
