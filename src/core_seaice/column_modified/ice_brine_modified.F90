!  SVN:$Id: ice_brine_modified.F90 1008 2015-06-20 23:55:12Z eclare $
!=======================================================================
!
...
!
      module ice_brine_modified

      use ice_kinds_mod_modified
      use ice_constants_colpkg_modified
      use ice_colpkg_tracers_modified, only: ntrcr, nt_qice, nt_sice, nt_bgc_S 
...
! may be > 1 in which case there is brine above the ice surface (meltponds). 
      ...
      if (fbri <= c0) then
         ...
         call add_warning(warning)         
         l_stop = .true.
         stop_label = 'ice_brine_modified preflushing: fbri <= c0'
      endif
...
! average ice permeability and the surface ice porosity
      ...
      use ice_therm_mushy_modified, only: permeability
      use ice_mushy_physics_modified, only: temperature_mush, liquid_fraction
      use ice_colpkg_shared_modified, only: l_sk, min_salin
...
!=======================================================================
      ...
      use ice_colpkg_shared_modified, only: rhosi
...
! 
      ...
      use ice_therm_shared_modified, only: calculate_Tin_from_qin
      use ice_colpkg_tracers_modified, only: nt_fbri, nt_Tsfc
      use ice_colpkg_shared_modified, only: min_salin, rhosi, salt_loss
      ...
      if (l_stop) then
         stop_label = 'CICE ice_brine_modified:zsalin < min_salin'
      endif
