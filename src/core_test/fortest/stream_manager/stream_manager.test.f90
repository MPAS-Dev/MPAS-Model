...
contains
    ...
    subroutine test_stream_mgr_write(f_ptr, ts_ptr, s_ptr, param_idx) bind(C)
        ...
        if (param_idx >= 1 .and. param_idx <= 7) then
            call MPAS_stream_mgr_create_stream(f%manager, stream_in,  MPAS_STREAM_INPUT,  'input.$Y-$M-$D_$h.$m.$s.nc',clobberMode=MPAS_STREAM_CLOBBER_OVERWRITE, ierr=ierr)
            call assert_equal(ierr, MPAS_STREAM_MGR_NOERR, verbosity=2)

            call MPAS_stream_mgr_create_stream(f%manager, stream_out, MPAS_STREAM_OUTPUT, 'output.$Y-$M-$D_$h.$m.$s.nc', clobberMode=MPAS_STREAM_CLOBBER_OVERWRITE, ierr=ierr)
            call assert_equal(ierr, MPAS_STREAM_MGR_NOERR, verbosity=2)
    ...
    subroutine test_stream_mgr_read(f_ptr, ts_ptr, s_ptr, param_idx) bind(C)
        ...
        !-----------------------------------------------------------------------
        if (param_idx >= 1 .and. param_idx <= 7) then
            call MPAS_stream_mgr_create_stream(f%manager, stream_in,  MPAS_STREAM_INPUT,  'test_output_alarm_stream.nc', ierr=ierr)
            call assert_equal(ierr, MPAS_STREAM_MGR_NOERR, verbosity=2)
    ...
    subroutine test_validate_streams(f_ptr, ts_ptr, s_ptr, param_idx) bind(C)
        ...
        type(stream_manager_fixture_t), pointer :: f
        integer :: ierr
        character(len=StrKIND) :: stream_in1, stream_in2
        character(len=StrKIND) :: alarm_in1, alarm_in2
        ...
        !-----------------------------------------------------------------------
        if (param_idx >= 1 .and. param_idx <= 7) then
            call MPAS_stream_mgr_create_stream(f%manager, stream_in1,  MPAS_STREAM_INPUT,  'stream1.nc',clobberMode=MPAS_STREAM_CLOBBER_OVERWRITE, ierr=ierr)
            call assert_equal(ierr, MPAS_STREAM_MGR_NOERR, verbosity=2)
            call MPAS_stream_mgr_create_stream(f%manager, stream_in2, MPAS_STREAM_INPUT, 'stream2.nc', clobberMode=MPAS_STREAM_CLOBBER_OVERWRITE, ierr=ierr)
            call assert_equal(ierr, MPAS_STREAM_MGR_NOERR, verbosity=2)

            ! Add alarms to both streams
            call mpas_add_clock_alarm(f%manager%streamClock, alarm_in1, f%clock_start_time, f%clock_time_step, ierr=ierr)
            call assert_equal(ierr, 0, verbosity=2)
            call MPAS_stream_mgr_add_alarm(f%manager, stream_in1, alarm_in1, MPAS_STREAM_INPUT, ierr=ierr)
            call assert_equal(ierr, MPAS_STREAM_MGR_NOERR, verbosity=2)
            call mpas_add_clock_alarm(f%manager%streamClock, alarm_in2, f%clock_start_time, f%clock_time_step, ierr=ierr)
            call assert_equal(ierr, 0, verbosity=2)
