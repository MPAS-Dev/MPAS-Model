...
contains
  ...
  SUBROUTINE noahmp_urban( NoahmpIO,sf_urban_physics,         NSOIL,     IVGTYP,ITIMESTEP,  & ! IN : Model configuration 
                             ...
                             SWDDIR,         SWDDIF,                                        &
                                GLW,          P8W3D,         RAINBL,       DZ8W,      ZNT, Z0,  & ! IN : Forcing
              ...
              FVEGXY,Q2MVXY,Q2MBXY,T2MVXY,T2MBXY, Q2,T2,TH2,U10,V10 & !,PSIM,PSIH,GZ1OZ0,AKHS,AKMS !cye-add 2025-03-26
                 )
  ...
  USE MPAS_KIND_TYPES !cye add 22/01/25
  USE mpas_atmphys_constants, only: KARMAN=>karman, CP=>cp, XLV=>xlv, RCP=>rcp !cye add 22/01/25
...
! output variable lsm --> surface_driver
     ...
     REAL,           DIMENSION( ims:ime, jms:jme ), INTENT(OUT) :: UST_URB2D
      ...
      REAL, OPTIONAL, DIMENSION( ims:ime, jms:jme ), INTENT(IN) :: FVEGXY,Q2MVXY,Q2MBXY,T2MVXY,T2MBXY !cye-add 2025-03-26
      REAL, OPTIONAL, DIMENSION( ims:ime, jms:jme ), INTENT(INOUT) ::Q2,T2,TH2,U10,V10 !,PSIM,PSIH,GZ1OZ0,AKHS,AKMS !cye-add 2025-03-26
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! ZNT(I,J)      = ZNT_URB    !cye add 2025-03-20
    ! Z0(I,J)       = ZNT_URB    !cye add 2025-03-20
      ...
      T2(I,J)  = (FVEGXY(I,J)*T2MVXY(I,J) + (1.-FVEGXY(I,J))*T2MBXY(I,J))*(1.-FRC_URB2D(I,J)) +   &
                  (TH2_URB2D(i,j)/((1.E5/P8W3D(I,KTS,J))**RCP))*FRC_URB2D(I,J)
      TH2(I,J) = T2(i,j)*(1.E5/P8W3D(I,KTS,J))**RCP
