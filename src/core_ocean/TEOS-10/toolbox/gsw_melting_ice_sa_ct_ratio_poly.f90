...
character (*), parameter :: func_name = "gsw_melting_ice_sa_ct_ratio_poly"

ctf = gsw_ct_freezing_poly(sa,p,saturation_fraction)
if (ct .lt. ctf) then
    ! the seawater ct input is below the freezing temperature
    gsw_melting_ice_sa_ct_ratio_poly = gsw_error_code(1,func_name)
    return
end if

tf = gsw_t_freezing_poly(0.0_r8,p,saturation_fraction)
if (t_ih .gt. tf) then
    ! t_ih exceeds the freezing temperature at sa = 0
    gsw_melting_ice_sa_ct_ratio_poly = gsw_error_code(2,func_name)
    return
end if
...
gsw_melting_ice_sa_ct_ratio_poly = sa*h_hat_ct/(h - h_ih - sa*h_hat_sa)
