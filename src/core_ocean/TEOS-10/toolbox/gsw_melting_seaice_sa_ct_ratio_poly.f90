...
use gsw_mod_toolbox, only : gsw_sa_freezing_from_t_poly, gsw_enthalpy_ice
use gsw_mod_toolbox, only : gsw_ct_from_t, gsw_t_freezing_poly
...
real (r8) :: ctf, delsa, h, h_brine, h_ih, sa_brine, ct_brine
real (r8) :: tf_sa_seaice, h_hat_sa, h_hat_ct

real (r8), parameter :: saturation_fraction = 0.0_r8

character (*), parameter :: func_name = "gsw_melting_seaice_sa_ct_ratio_poly"

if (sa_seaice .lt. 0.0_r8 .or. sa_seaice .gt. 15.0_r8) then
    gsw_melting_seaice_sa_ct_ratio_poly = gsw_error_code(1,func_name)
    return
end if

ctf = gsw_ct_freezing_poly(sa,p,saturation_fraction)
if (ct .lt. ctf) then    ! the seawater ct input is below the freezing temp
    gsw_melting_seaice_sa_ct_ratio_poly = gsw_error_code(2,func_name)
    return
end if

!--------------------------------------------------------------------------
tf_sa_seaice = gsw_t_freezing_poly(sa_seaice,p,saturation_fraction) - 1e-6_r8
if (t_seaice .gt. tf_sa_seaice) then   ! t_seaice exceeds the freezing sa
    gsw_melting_seaice_sa_ct_ratio_poly = gsw_error_code(3,func_name)
    return
end if
!
...
if (sa_brine .gt. gsw_error_limit) then
    gsw_melting_seaice_sa_ct_ratio_poly = gsw_error_code(4,func_name,sa_brine)
    return
end if
...
gsw_melting_seaice_sa_ct_ratio_poly = h_hat_ct*delsa / &
