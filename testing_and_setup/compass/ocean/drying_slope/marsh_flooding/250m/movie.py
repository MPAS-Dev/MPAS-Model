...
#                      marker = '.', color = 'g', label='ROMS')
        if frame:
            ...
            plot_MPASO(timeslice, fileno, 'k-', lw=2, label=labels[int(fileno)-1])
        else:
          plot_MPASO([dtime], fileno, 'k-', lw=0.5, label=labels[int(fileno)-1])
...
# subplot r = 0.0025
    ...
    for atime  in np.linspace(0,0.5,5*12+1):
        ...
        # subplot r = 0.0025
        upper_plot()
        timestr = plot_MPASO([atime], '1', 'k-', lw=2.0, label='Lmarsh=2.5km')
